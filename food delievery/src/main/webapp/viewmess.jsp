...
<body>
	...
	<h3>
		<b><center>View Mess's</center></b>
	</h3>
	<table class="table table-striped table-bordered table-hover" style="width:100%;margin:20px;">
		<tr>
			<th>id</th>
			<th>name</th>
			<th>description</th>
		...
		<tr>
			<%
				...
				String sql = "select * from addmess where messorres='MESS' group by name";
			...
			<td><img src="<%=rs.getString(5)%>"
				...
				<td><a href="viewMessByName.jsp?n=<%=rs.getString(2)%>">View</a></td>
			<td><a href="adminviewmessupdate.jsp?id=<%=rs.getString(1)%>">Update</a></td>
			<td><a href="adminviewmessdelete.jsp?id=<%=rs.getString(1)%>&mr=<%=rs.getString(7)%>">Delete</a></td>
			<%
